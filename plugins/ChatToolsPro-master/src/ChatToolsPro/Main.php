...
return true;
        ...
        elseif($args[0] == "4"){
         ...
         elseif($args[0] == "5"){
                ...
                case "announcement":
                $sender->sendMessage("Ã‚Â§aMessage announced sucessfully!");
                $sender->getServer()->broadcastMessage(TextFormat::AQUA . "[Announcement] " . implode(" ", $args));
                return true;
             
                case "serversay":
                $sender->getServer()->broadcastMessage(TextFormat::LIGHT_PURPLE . "§3[§bL§aO§6G §eᗰÁYᑕᕼỦ§3]§c⭐»--«⭐§a " . implode(" ", $args));
                return true;
                case "staffsay":
		...
		$player = $sender->getServer()->getPlayer($name);
			...
			return \true;
                ...
                if($player instanceof Player){
			$sender->sendMessage(TextFormat::DARK_RED . "[Warning" . " -> " . $player->getDisplayName() . "] " . "Ã‚Â§c" . implode(" ", $args));
		...
		$player = $sender->getServer()->getPlayer($name);
			...
			$sender->sendMessage("Ã‚Â§cUsage: /tipgive <Player> <Tip>");
		}
                return true;
                case "hug":
                $name = \strtolower(\array_shift($args));
		...
		$player = $sender->getServer()->getPlayer($name);
			...
			$sender->sendMessage("Ã‚Â§cUsage: /hug <Player>");
		}
                return true;
                case "opfake":
                $name = \strtolower(\array_shift($args));
