            ...
            if(is_numeric($args[0])){
                if(strpos($args[0], ".") !== false){
                    ...
                    if($sender->y + Provider::INVENTORY_HEIGHT > Level::Y_MAX){
                        ...
                        if($sender->hasPermission("playervaults.vault.".$args[0])){
                            $sender->sendMessage(TF::YELLOW."Đang mở két sắt số ".TF::AQUA."".$args[0]."...");
                ...
                if($sender->isOp()){
                    switch(strtolower($args[0])){
                        case "of":
                            if(!isset($args[1])){
                                $sender->sendMessage(TF::RED."Xin hãy sử dụng: /$cmd of <Tên Người Chơi> <Tên Két Sắt>");
                            }else{
                                ...
                                if(!is_numeric($args[2])){
                                    $sender->sendMessage(TF::RED."Xin hãy sử dụng: /$cmd of <Tên Người Chơi> <1-".$this->getMaxVaults().">");
                                    break;
                                }
                                $this->getData()->sendContents($args[1], $args[2] ?? 1, $sender->getName());
                                $sender->sendMessage(TF::YELLOW."Đang Mở Két Sắt Số ".TF::AQUA."".($args[2] ?? 1)." cho ".($player ?? $args[1])."...");
                            }
                            break;
                        case "empty":
                            if(!isset($args[1])){
                                $sender->sendMessage(TF::RED."Xin hãy sử dụng: /$cmd empty <Teen Người Chơi> <Teen Két Sắt|all>");
                            }else{
